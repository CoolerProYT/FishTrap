...
import { data, seconds } from '../fishtrap'
import ItemSlot from './ItemSlot.vue'

const props = defineProps<{ id: string }>()

const recipe = computed(() => data.recipes.find((r) => r.id === props.id || r.id === `fishtrap:${props.id}`))
...
<template>
  <div v-if="recipe" class="ft-recipe">
    ...
    <template v-else-if="recipe.ingredient">
  ...
  <p v-else class="ft-muted">Recipe {{ id }} not found.</p>
</template>
